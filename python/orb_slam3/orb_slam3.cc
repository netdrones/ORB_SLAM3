...
py::array_t<double> System::TrackStereo(const py::array_t<uint8_t>& imLeft,
  ...
  if (bil.format != py::format_descriptor<uint8_t>::format()) {
    throw std::runtime_error("left image dtype must be uint8");
  }
  if (bir.format != py::format_descriptor<uint8_t>::format()) {
    throw std::runtime_error("right image datat type be uint8");
  }

  auto bi_imu = imuMeas_.request();
  if (bi_imu.shape[1] != 7) {
    throw std::runtime_error("IMU must have 7 elements");
  }
  ...
  for (auto i = 0; i < bi_imu.shape[0]; i++) {
    ...
    auto acc_x = static_cast<float>(data[7 * i]);
    auto acc_y = static_cast<float>(data[7 * i + 1]);
    auto acc_z = static_cast<float>(data[7 * i + 2]);
    auto vel_x = static_cast<float>(data[7 * i + 3]);
    auto vel_y = static_cast<float>(data[7 * i + 4]);
    auto vel_z = static_cast<float>(data[7 * i + 5]);
    auto timestamp = static_cast<double>(data[7 * i + 6]);
    imuMeas[i] = ::ORB_SLAM3::IMU::Point(
      acc_x,
      acc_y,
      acc_z,
      vel_x,
  ...
  auto Tcw = slam_.TrackStereo(
    cv::Mat(bil.shape[0], bil.shape[1], bil.shape[2], reinterpret_cast<uint8_t*>(bil.ptr)),
    cv::Mat(bir.shape[0], bir.shape[1], bir.shape[2], reinterpret_cast<uint8_t*>(bir.ptr)),
    timestamp,
    imuMeas
  );
  ...
  auto twc_ary = py::array_t<double>(7);
  auto bi_twc = twc_ary.request();
  auto p = static_cast<double *>(bi_twc.ptr);
  p[0] = twc(0);
  p[1] = twc(1);
  p[2] = twc(2);
  p[3] = q.x();
